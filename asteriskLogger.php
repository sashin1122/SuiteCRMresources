...
// this is required to skip input sanitazing done by entry point...
  ...
  //  $asteriskManagerPort = 5038;
    */
    ...
    if (!$amiSocket) {
        if ($sendemail == true) {
            $sendemail = false;
            if (substr(date('Hi'),0,3) !== '050') {
				mail( 'ettienne.vosloo@flightscope.com', 'AMI VoIP.flightscope.co.za Failed at ' . date( "Y-m-d H:i:s" ) . '!', 'AMI connection on Asterisk_Logger in SugarCRM has failed at ' . date( "Y-m-d H:i:s" ) . '!' );
            }
        }
        ...
        if ($sendemail == false) {
            if (substr(date('Hi'),0,3) !== '050') {
				mail( 'ettienne.vosloo@flightscope.com', 'AMI working again at ' . date( "Y-m-d H:i:s" ) . '', 'AMI connection on Asterisk_Logger in SugarCRM has been succesfully opened after failure at ' . date( "Y-m-d H:i:s" ) . '!' );
            }
        }
        $sendemail = true;
    }
...
// if logging is enabled.
    if (!empty($sugar_config['asterisk_log_file']) && !empty($logFile)) {
        ...
        if (file_exists($myFile) == false && is_writable(dirname($myFile)) == false){
            print "Error: unable to create file $myFile: ". '\n';
            return;
        }
        else if (file_exists($myFile) && is_writable($myFile) == false){
            print "Error: unable to write to file $myFile: ". '\n';
            return;
        }

        $fh = fopen($myFile, 'a');
        if ($fh === false){
            print "Error: unable to logLine to $myFile: ". '\n';
        }
        else{
            fwrite($fh, $str);
            fclose($fh);
